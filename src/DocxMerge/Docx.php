...
class Docx
{
    ...
    public function addFile($filePath, $zipName, $refID, $addPageBreak = false): void
    {
    ...
    private function addReference($zipName, $refID): void
    {
    ...
    private function addAltChunk($refID, $addPageBreak): void
    {
    ...
    private function addContentType($zipName): void
    {
    ...
    public function findAndReplace($key, $value): void
    {
        // Apply styles
        if (is_array($value)) {
    ...
    /* Add styles for text */
    private function findAndReplaceWithStyles($key, $value): void
    {
        $lastPos = 0;
        $positions = array();
    ...
    /* end */
    public function findAndReplaceFirst($key, $value): void
    {
        // Search/Replace first key in document
        if (strpos($this->docxDocument, $key) === false) {
            return;
        }
        if (strpos($this->docxDocument, $key) + strlen($key) === false) {
            return;
        }
    ...
    public function flush(): void
    {
        ...
        } else if (copy($tempFile, $this->docxPath)) {
            unlink($tempFile);
        }
    }

    public function writeContent($content, $zipPath): int
    {
        $this->docxZip->FileReplace($zipPath, $content, TBSZIP_STRING);
        return 0;
    }

    public function copyRowWithPlaceholder($placeholder, $N): void
    {
        $needle = "\${" . $placeholder . "}";
